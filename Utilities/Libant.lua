...
local old_OwO = CoreGui:FindFirstChild('OwO')

if old_OwO then
    Debris:AddItem(old_OwO, 0)
end
...
function Library:create_ui()
    ...
    local OwO = Instance.new('ScreenGui')
...
gradient.Color = ColorSequence.new({
    ColorSequenceKeypoint.new(0.00, Color3.fromRGB(20, 5, 15)),      -- Rosa escuro
    ColorSequenceKeypoint.new(0.25, Color3.fromRGB(40, 15, 30)),     -- Rosa médio escuro
    ColorSequenceKeypoint.new(0.50, Color3.fromRGB(70, 25, 50)),     -- Rosa médio
    ColorSequenceKeypoint.new(0.70, Color3.fromRGB(100, 40, 75)),    -- Rosa médio claro
    ColorSequenceKeypoint.new(0.85, Color3.fromRGB(130, 60, 100)),   -- Rosa claro
    ColorSequenceKeypoint.new(0.95, Color3.fromRGB(160, 80, 125)),   -- Rosa bem claro
    ColorSequenceKeypoint.new(1.00, Color3.fromRGB(190, 100, 150))   -- Rosa mais claro
})
...
gradient.Parent = Container
    ...
    UIStroke.Color = Color3.fromRGB(255, 254, 255) -- Color3.fromRGB(52, 66, 89)
    ...
    ClientName.TextColor3 = Color3.fromRGB(255, 254, 255)
    ClientName.TextTransparency = 0.20000000298023224
    ClientName.Text = 'UwU.dll'
...
Icon.ImageColor3 = Color3.fromRGB(255, 254, 255)
...
if Icon and Icon:IsA("ImageLabel") then
    RunService.RenderStepped:Connect(function()
        ...
        if currentTime - lastFrameTime >= FRAME_DURATION then
            Icon.Image = "rbxassetid://" .. imageIds[currentFrame]
            currentFrame += 1
            if currentFrame > #imageIds then
                currentFrame = 1
            end
    ...
    getgenv().guilibraryVisible = false

    local function setBlurTransparency(state)
        local blurFolder = workspace.CurrentCamera:FindFirstChild("AcrylicBlur")
        if blurFolder then
            local root = blurFolder:FindFirstChild("Root")
            if root then
    ...
    function self:load()
        ...
        for _, object in OwO:GetDescendants() do
    ...
    function self:create_tab(title: string, icon: string)
        ...
        function TabManager:create_module(settings: any)
            ...
            local Icon = Instance.new('ImageLabel')
            Icon.ImageColor3 = Color3.fromRGB(255, 181, 255)
            Icon.ScaleType = Enum.ScaleType.Fit
            ...
            function ModuleManager:create_text(settings: any)
                ...
                if not settings.rich then
                    ...
                    Body.RichText = true
                    Body.Text = settings.richtext or "<font color='rgb(255,0,0)'>OwO</font> user" -- Default rich text
                end
                ...
                function TextManager:Set(new_settings)
                    if not new_settings.rich then
                        ...
                        Body.RichText = true
                        Body.Text = new_settings.richtext or "<font color='rgb(255,0,0)'>OwO</font> user" -- Default rich text
                    end
                end;
            ...
            function ModuleManager:create_dropdown(settings: any)
                ...
                function DropdownManager:set_options(newOptions: table)
    ...
    if typeof(current) == "string" and table.find(newOptions, current) then
